...
ps -ef|grep deploy.sh|grep -v grep|egrep -v   "mrte1\s+$$\s+"
	echo "script zaten calisiyor."
	exit
fi
...
if [[ -s chk1 ]]
then
doMail "basda si-check-processes hatasi: ilk hata:   $(ssh bas@10.248.68.128 'source .profile;si-check-processes' 2>&1|grep :E:|head -1 ).  1 dakika icinde unixde ctrl+c basilmazsa deploya baslanacak. Sleeping 1mins"
