...
ssh mrte2@localhost 'source .profile;chmod +x $ORGADATA/CmRoot/*.sh;chmod +x $ORGADATA/CmRoot/*.py ' 
...
ssh bas@10.248.68.128 'source .profile;si-check-orgadata --fix-svn;echo ---;cs_client "Update Agile$(date +%Y%m%d%H%M)" submit;echo ---;oupm_start -N'
date
...
echo Install Subscriber for test: >>out.txt
 ...
 echo -e "Finished, but product-cache is in progress; it may last 1 minute" >> out.txt
