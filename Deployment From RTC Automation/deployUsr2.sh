...
if    ps -ef|grep deploy.sh|grep -v grep|egrep -v   "mrte1\s+$$\s+"|egrep -q  deploy.sh
then
#ps -ef|grep deploy.sh|grep -v grep
ps -ef|grep deploy.sh|grep -v grep|egrep -v   "mrte1\s+$$\s+"|grep -v $$
	echo "script zaten calisiyor."
	exit
fi
...
then
        echo Patch Testi yapiliyor. Bitince deneyiniz ya da DEV ortamini kullaniniz. Developerlarin test ortamini kullanmamasini rica ederiz.
        exit
fi
...
if [ ! -z "$EMAIL_LIST" ]
then
ssh mrte1@10.248.68.122 "source .profile;echo -en \" Starting to deploy to Agile Environment from $(grep RTC_TST1_STREAM /rte/orgadata/mrte1/avea/work/AveaRtcScmTools/scripts/rtc_refresh_gold_config.properties) \\n 5 dakika RTEde 10dk BASda kesinti bekleniyor. \\n \\n Mrtelerde, sadece bu processler restartlanacak: \\n $(cat mrte_restart_komutlari.txt|xargs echo|grep -oP "\-K [^ ]* "|cut -d' ' -f2|xargs echo) \\n \\n Processlere ekleme yapmak icin /tmp/deploy/mrte_restart_komutlari.txt dosyasina giriniz. \\n Dosya sonradan bash /tmp/deploy/mrte_restart_komutlari.txt seklinde manuel de kosulabilir.\"  | mailx -s 'Agile ortamina Deployment Basliyor ^_^' $EMAIL_LIST"
fi
done <.mail
...
#cd -
  ...
  #önemli bir hata olmadýðý halde hata basarsa if den fi ye  7 satiri silip kaldiriniz.
...
ssh mrte2@localhost 'source .profile;chmod +x $ORGADATA/CmRoot/*;chmod +x $ORGADATA/CmRoot/*.py;chmod +x $ORGAROOT/si/scripts/gcp.pl;cp $ORGADATA/cnf/extComm/config.cfg $ORGADATA/cnf/extComm/config.cfg.si-check-orgadatadanGelen;cp $ORGADATA/cnf/extComm/config.cfg.PRE_060 $ORGADATA/cnf/extComm/config.cfg ' 
#ssh mrte2@localhost 'source .profile;mv $ORGADATA/cnf/reg/scp/zones/cf.old $ORGADATA/cnf/reg/scp;' 
#oupm_start -N&
#ssh mrte2@localhost 'source .profile;oupm_start -N' &
ssh mrte2@localhost 'source .profile;cd /tmp/deploy;bash mrte_restart_komutlari.txt'  
...
then
 ...
 ssh bas@10.248.68.128 'source .profile;oupm_start -N'
 cleanup;exit
fi
...
echo mrte1:
si-check-processes
si-check-processes 2>&1 |grep :E:|grep -v lic_mngr > chk1
if [[ -s chk1 ]]
then
doMail "mrte1de si-check-processes hatasi: ilk hata:  $(si-check-processes 2>&1|grep :E:|grep -v lic_mngr|head -1).  1 dakika icinde unixde ctrl+c basilmazsa ya da kill yapilmazsa deploya baslanacak. Sleeping 1 min "
echo "mrte1de si-check-processes hatasi:    $(si-check-processes 2>&1).  1 dakika icinde unixde ctrl+c basilmazsa ya da kill yapilmazsa deploya baslanacak. Sleeping 1 mins"
patlak=$(si-check-processes 2>&1|grep :E:|head -1| sed "s|.* \([^\s]*.xml\).*|\1|")
if [ -n $patlak ]
then
	xmllint --format $ORGADATA/$patlak 2> patlak 1>/dev/null
	if [ -s patlak ]
	then
		scp patlak mrte1@10.248.68.122:/tmp
		cat maillist.txt |xargs echo > .mail
		while read EMAIL_LIST
		do
			if [ ! -z "$EMAIL_LIST" ]
			then
			ssh mrte1@10.248.68.122 "source .profile;cat  /tmp/patlak| mailx -s 'AUTo Defect Finder - Agile Ortami' $EMAIL_LIST"
			fi
		done < .mail
	fi
fi
...
ssh bas@10.248.68.128 'source .profile;si-check-processes' 2>&1 |grep :E:|grep -v lic_mngr > chk1
if [[ -s chk1 ]]
then
doMail "basda si-check-processes hatasi: ilk hata:   $(ssh bas@10.248.68.128 'source .profile;si-check-processes' 2>&1|grep :E:|grep -v lic_mngr|head -1 ).  1 dakika icinde unixde ctrl+c basilmazsa deploya baslanacak. Sleeping 1mins"
echo "basda si-check-processes hatasi:    $(ssh bas@10.248.68.128 'source .profile;si-check-processes'  ).  1 dakika icinde unixde ctrl+c basilmazsa deploya baslanacak. Sleeping 1mins"
sleep 60
fi

echo mrte2:
ssh mrte2@localhost 'source .profile;si-check-processes' 
ssh mrte2@localhost 'source .profile;si-check-processes' 2>&1 |grep :E:|grep -v lic_mngr > chk1
if [[ -s chk1 ]]
then

doMail "mrte2de si-check-processes hatasi:  son hata:  $(ssh mrte2@localhost 'source .profile;si-check-processes' 2>&1|grep :E:|grep -v lic_mngr|tail -1)  1 dakika icinde unixde ctrl+c basilmazsa ya da kill yapilmazsa deploya baslanacak. Sleeping 1 min "
echo "mrte2de si-check-processes hatasi:    $(ssh mrte2@localhost 'source .profile;si-check-processes' 2>&1).  1 dakika icinde unixde ctrl+c basilmazsa ya da kill yapilmazsa deploya baslanacak. Sleeping 1 mins"
sleep 60
fi
...
chmod +x $ORGADATA/CmRoot/*.py
	...
	xmllint --format /rte/orgaroot/mrte1/si/bas/orgadata/cnf/domain-properties/basnc/productConfig/RecurringProduct.xml 2> patlak 1>/dev/null
	if [ -s patlak ]
	then
		scp patlak mrte1@10.248.68.122:/tmp
		cat maillist.txt |xargs echo > .mail
		...
		do
			if [ ! -z "$EMAIL_LIST" ]
			then
			ssh mrte1@10.248.68.122 "source .profile;cat  /tmp/patlak| mailx -s 'AUT Defect Finder - Agile Ortami' $EMAIL_LIST"
			fi
		done < .mail
	fi
	
	xmllint --format /rte/orgaroot/mrte1/si/bas/orgadata/cnf/domain-properties/basnc/productConfig/NonRecurringProduct.xml 2> patlak 1>/dev/null
	if [ -s patlak ]
	then
		scp patlak mrte1@10.248.68.122:/tmp
		cat maillist.txt |xargs echo > .mail
		while read EMAIL_LIST #AUT TODO: Change while to if as it is a 1 time loop
		do
			if [ ! -z "$EMAIL_LIST" ]
			then
			ssh mrte1@10.248.68.122 "source .profile;cat  /tmp/patlak| mailx -s 'AUT Defect Alarm - Agile Ortami' $EMAIL_LIST"
			fi
		done < .mail
	fi

echo -e Merhaba\\n\\nLoglar "/tmp/deploy" klasorunde. Hata varsa acil mudahale ediniz > out.txt
...
echo >>out.txt
 if  grep -q ":P:Could not deploy " out.txt 
 then
 grep INT.csi  mrte1OUPMLog.txt|egrep :[WEP]:|egrep -v ":E:Configure: Config selection failed|config_19700101000000.xml]|/rte/orgadata/mrte1/wrk/orgadata_csi/cnf/int/01/rte_interface.xml]|/rte/orgadata/mrte1/wrk/orgadata_csi/cnf/cre/config.dir.xml" >out2.txt
	echo >>out2.txt
	echo Mrte Loglarindaki Hatalar:>>out2.txt
	egrep :[WEP]: mrte2OUPMLog.txt mrte1OUPMLog.txt|egrep -vi 'sql|onnect|eocn|Request set |subscriber|not changed to realtime:'>>out2.txt
#	echo _________________________________________________________________________________>>out2.txt
	echo AUT>>out2.txt
	
	 scp out2.txt mrte1@10.248.68.122:/tmp
	  cat maillist.txt |xargs echo > .mail
while read EMAIL_LIST
do
if [ ! -z "$EMAIL_LIST" ]
then
ssh mrte1@10.248.68.122 "source .profile;cat  /tmp/out2.txt| mailx -s 'Deployment Hatalari - Agile Ortami' $EMAIL_LIST"
fi
done < .mail
 fi
echo Terminate a subscriber for test: >>out.txt 2>&1
...
echo
egrep 'Commiting.to.+done' bascs_01_0?_CSLOG.txt >> out.txt
echo
 echo  Finished. product cache and refresh domain-properties may be still in progress for the next 3 mins. >> out.txt
