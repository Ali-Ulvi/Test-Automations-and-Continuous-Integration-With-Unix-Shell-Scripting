...
doMail "yenilemeSMSleri.txt dosyasindaki sms sayisi servisID_Fiyat.txt dosyasindaki servis sayisinin 4 kati olmalý. $(( $(wc -l .servisIDler.txt |awk '{print $1}') * 4 )) -ne $(wc -l .yenilemeSMSleri.txt |awk '{print $1}') TESTE BASLANAMADI."
exit 1
fi



if [[ $(egrep 'ð|ü|ý|þ|ç|ö|Ö|Ç|Þ|Ý|Ð|Ü|’' .yenilemeSMSleri.txt|wc -l) -ne 0 ]]
then
echo Turkce Karakterli veya ters tirnak isaretli  satirlar:
egrep 'ð|ü|ý|þ|ç|ö|Ö|Ç|Þ|Ý|Ð|Ü|’' .yenilemeSMSleri.txt
echo
...
cat .yenilemeSMSleri.txt| tr 'ðüýþçöÖÇÞÝÐÜ’' "guiscoocsigu'" > tmp$msisdn
