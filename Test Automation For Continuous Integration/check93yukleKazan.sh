...
saat=23:59
tarih2=$gun\/$ay\/$yil
sms=$(echo "$sms"|sed "s|<TARIH>|$tarih2|g"|sed "s|<SAAT>|$saat|g" |sed "s|HH:MI|$saat|g"|sed "s|DD\/MM\/YYYY|$tarih2|g"|sed "s|’|'|g"|sed 's/^ //'|sed 's/ $//'|sed 's/\.$//'|sed "s|*|\\\*|g"|sed "s|\.|\\\.|g"|sed -e "s|(|\\\(|g"|sed -e "s|\[|\\\[|g"|sed -e "s|\]|\\\]|g"|sed -e "s|)|\\\)|g")
#project finished
...
echo BASARILI
 ...
 if [[ `egrep -v '<MSGID>1</MSGID>|<MSGID>6384</MSGID>|^$|><MSGID>3</MSGID>|<MSGID>4</MSGID>|<MSGID>1833</MSGID>|<MSGID>18..</MSGID>'  sms9333Log.$pid.$(date +%Y%m%d)*|grep $msisdn|grep -oP "(?<=<MSGID>)[^<]+"|wc -l` -gt 2 ]] 
    then 
	grep ${msisdn} sms9333Log.$pid.$(date +%Y%m%d)*    
