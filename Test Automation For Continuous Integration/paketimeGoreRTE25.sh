...
# AUT Inventions (R)
#imlicit desicion matrixRTE creator

testBilgi="$0 $@"
echo start > goreRTE 
date >> goreRTE
####VARIABLES ARENA###
flag25=BLC_PaketimeGore25tlFlagBonus_03351      #
ntf25=BLC_PaketimeGore_1GB25TLNotifyBonus_03352    #
######################
...
varsa()
{
if grep -Fxq "$1" verilecekBonuslarRTE
then
...
if [[ "$bonusSayisi2" -ne $(expr 8 + "$bonusSayisi1") ]]
then
	echo -e "\e[7m  bonusSayisi2 wrong  ${NC}"
	doMail " bonus sayisi degisti. önce:$bonusSayisi1 sonra:$bonusSayisi2 check RTEEvents.xml"
...
then
	echo -e "\e[7m  bonusSayisi2 wrong  ${NC}"
	doMail "NoWin bonus sayisi degisti. önce:$bonusSayisi1 sonra:$bonusSayisi2 check RTEEvents.xml"
	echo önce:$bonusSayisi1 sonra:$bonusSayisi2
	subsquery $msisdn b
fi
}
...
./checkBonus  $msisdn BLB_PaketimeGore_1GBBonus_02770 1073741824 Bytes 1 $gun
./checkBonus  $msisdn BLC_GenericData_DiscountPromoBonus_00452 1 CNT 1 $gun
./checkBonus  $msisdn $ntf25 1 CNT 1 0
checkCount
}
...
if varsa $flag25; then 
echo check25
check25
else     
echo code if not found 25
checkCountNoWin 
fi 
}
#void main
...
rm matrixRTE
#loading decision matrixRTE:
for ((i=0;i<${#c1[@]};i++)) do
    for ((j=0;j<${#c2[@]};j++)) do
		for ((k=0;k<${#c3[@]};k++)) do
			for ((l=0;l<${#c4[@]};l++)) do
				for ((m=0;m<${#c5[@]};m++)) do
					echo "${c1[i]} ${c2[j]} ${c3[k]} ${c4[l]} ${c5[m]}" >> matrixRTE
					#matrixRTE[$i,$j]=$RANDOM
				done
