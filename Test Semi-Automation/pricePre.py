...
def printUsagePrice (cntv):
        for price in cntv:
                ...
                ####if (premium is None ):
				####	continue
                str= tariff.attrib['name'] +" usageRule: "   + usageRule.attrib['name'] +"\tprice: "+ price.text +" unit size: " +      price.getparent().find('unitSize').text  +" fixedUnit: " +      price.getparent().find('fixedUnit').text     +" fixedCost: " +      price.getparent().find('fixedCost').text     + " " +   price.getparent().find('eventClass').text 
...
tre  = et.parse(open("/rte/orgaroot/mrte1/si/cs/subversion/branches/OPSC-CONF-Avea_r242/LOGICAL/ProductCatalog/ProductCatalogCore.xml"), parser)
